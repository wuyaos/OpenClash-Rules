...
const TARGETS = [
  {
    iniPath: path.join(ROOT, "config", "ACL4SSR_mod_mini.ini"),
    outPath: path.join(ROOT, "scripts", "flclash.acl.js"),
    label: "acl",
  },
  {
    iniPath: path.join(ROOT, "config", "Home_mod_mini.ini"),
    outPath: path.join(ROOT, "scripts", "flclash.home.js"),
    label: "home",
  },
];
...
function main() {
  ...
  for (const item of stats) {
    console.log(
      `[flclash:${item.label}] ${item.ini} -> ${item.out} (groups=${item.groups}, providers=${item.providers}, rules=${item.rules})`
    );
  }
}
